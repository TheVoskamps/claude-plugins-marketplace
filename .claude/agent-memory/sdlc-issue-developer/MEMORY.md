...
- [claude-vm real build+boot IS doable](claude-vm-real-build-and-boot-is-doable.md) — start podman machine yourself and verify container network first (a stopped-ish machine looks like a broken diff); inspect the .raw with debugfs, not mount; probe arm64 binaries in a plain container
- [Baked plugin changes are unverifiable pre-merge](baked-plugin-changes-unverifiable-pre-merge.md) — the guest bakes the marketplace's DEFAULT branch, so a feature branch can never be guest-tested; verify the layers instead and hand over a post-merge plan
