...
description: sdlc no longer reads .claude/rules/repo-config.md's full six-field contract directly; branch/PR mechanics moved into git-tools:git-branch-create (formerly gh:branch-create, moved by issue #166) and github-prs:pr-create, each doing its own inline 2-field parse (issue #143 / PR #150)
metadata:
  type: project
---
...
- `issue-developer` delegates branch creation to
  `/git-tools:git-branch-create` (this skill lived in a standalone
  `gh` plugin as `/gh:branch-create` until issue #166 / PR #167
  retired that plugin and moved the skill into `git-tools`) and PR
  creation to `/github-prs:pr-create` — both read their own 1-2
...
touches `sdlc` agent frontmatter, `orchestrate/SKILL.md`,
`git-tools`, or `github-prs`, do not assume an agent or the
orchestrator reads the full repo-config contract — check its
`skills:` frontmatter and "Read global rules first" section first. See
