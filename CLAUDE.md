...
- [`docs/config-file-conventions.md`](docs/config-file-conventions.md) —
  ...
  own. Kernel: never JSON — YAML, or Markdown with front-matter when
  a human needs the prose too, stamped `schema-version`, under
  `$XDG_CONFIG_HOME/<plugin>/`, or `$XDG_STATE_HOME` for run state.
