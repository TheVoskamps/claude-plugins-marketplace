...
// .tool_input / .cwd). A past regression in test-deny-gate.sh is a
...
func parseEvent(raw []byte) (*Event, error) {
	...
	// shipped across versions, so the subagent-conditioned rules fire
...
// main session). Used to scope the subagent-conditioned rules.
//
