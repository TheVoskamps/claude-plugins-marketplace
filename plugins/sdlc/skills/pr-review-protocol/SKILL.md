...
description: The operating protocol every sdlc PR reviewer runs — inputs, workflow, review criteria, finding format, verdict derivation, and end-of-run cleanup. Preloaded into each pr-reviewer variant via its skills frontmatter; not invoked from the user's slash menu.
user-invocable: false
---

# PR Review Protocol

This is the operating instruction for every `sdlc` PR reviewer. It is
**tier-invariant**: it carries no tier parameter and never asks which
reviewer is running it. The reviewer's reasoning tier lives solely in
the spawned agent's frontmatter `effort:`, and the orchestrator picks
a tier by choosing which agent definition to spawn (see the
`/sdlc:orchestrate` skill → "Picking a reviewer tier").

You do not write code — you only analyze, optionally exercise the
change in your worktree, and post a single structured review.
...
The protocol takes double-dash parameters. One vocabulary serves both
entry paths: the orchestrator's reviewer brief writes exactly these
tokens into the spawn prompt, and a direct invocation passes the same
flags on the command line. Read them from whichever of the two
delivered them.

- `--pr <N>` (required) — the pull request to review. With no `--pr`,
  stop and report that the brief named no PR rather than guessing one.
- `--issues <N…>` (optional) — the issue numbers this PR closes,
  space- or comma-separated, each with or without a leading `#`. This
  is your **claim**, not the answer: step 2 of the Workflow reconciles
  it against the branch. Absent, step 2 takes the claim from the PR
  body instead — the standalone path.
- `--branch <name>` (optional) — the PR's head branch. Absent, step 2
  reads it from GitHub.

No other parameter exists. In particular there is no tier, effort, or
model parameter: a brief that carries one is malformed, and you ignore
...
2. Then read this repo's `.claude/rules/repo-config.md` from the
   ...
   `Read` from an `sdlc` skill cannot resolve a path inside another
...
- `issue-link-prefix` (string, e.g. `"#"` for GitHub or `"SET-"` for
  ...
  `github-prs:pr-closing-issues` (declared in the reviewer's `skills:`
  frontmatter) read no repo-config at all — they are GitHub-only by
  design — and `git-tools:git-issues-from-branch` reads
...
though this protocol doesn't consume the whole contract).

In the rest of this document, `<link-prefix>` means the resolved value.

## Read the shared reviewer memory

Every reviewer variant declares `memory: project`, so its own captures
land in its own `.claude/agent-memory/<agent-name>/` directory — and
the curated reviewer lore lives in the **base** directory,
`.claude/agent-memory/sdlc-pr-reviewer/`. There is no sharing
mechanism between agent-memory directories, so read that base
directory yourself before reviewing, whichever variant you are:

```bash
cat .claude/agent-memory/sdlc-pr-reviewer/MEMORY.md
```

then read the entries whose one-line hooks look relevant, exactly as
you would your own. If the directory does not exist, there is no
shared lore yet — proceed. Your own captures still go to your own
directory (step 7 of the Workflow); `agent-memory-scrubber` curates
the whole tree, so a lesson you record as a variant reaches the base
directory through curation rather than by you writing there.

## Workflow

1. Fetch the PR diff via `/github-prs:pr-diff <--pr value>` (preloaded
   via the reviewer's `skills:` frontmatter).
2. Identify the **issue set** this PR is for. A PR delivers a
   **batch** — an ordered set of issues implemented on one branch —
   and a batch of one is the ordinary single-issue PR. When `--branch`
   was not supplied, fetch the branch name on GitHub with
   `gh pr view <PR> --json body,headRefName`; fetch the body the same
   way in either case. Then:

   - **Your claim** is `--issues`, when the brief supplied it. Run
     standalone on a bare `--pr` — the `/sdlc:git-review-pr` path —
     there is no issue set to take it from, so get it from
     `/github-prs:pr-closing-issues <PR>` (preloaded via the
     reviewer's `skills:` frontmatter), the one skill that reads a PR
   ...
   - **Reconcile the claim against the branch.** Invoke
     ...
     (preloaded via the reviewer's `skills:` frontmatter) — the one
   ...
   usual shape when `/sdlc:git-review-pr` hands you a bare `--pr` —
...
3. Read **each member independently** — every issue in the set you
   ...
   `/issue-view` skill (preloaded via the reviewer's `skills:`
   frontmatter and invoked through the `Skill` tool) rather than
   hand-rolling `gh issue view`. Do not rely solely on the
   orchestrator's spawn-brief summary — read each issue yourself so you
   can check the PR against that issue's own text:
...
6. Post your review via `/github-prs:pr-review-submit <PR>
   <verdict> <body>` (preloaded via the reviewer's `skills:`
   frontmatter), with `<verdict>` the **overall** verdict — one of
   `approve`, `request-changes`, or `comment`. The body carries a
   verdict per member plus that overall verdict; see "Per-issue
   verdicts, one overall" below for how the overall one is derived.
...
7. Capture agent memory onto the branch. `memory: project` resolves
   ...
   - Stage and commit **only** `.claude/agent-memory/` — never
     ...
     git commit -m "Add agent memory from <your agent name>"
     git push
     ```
   ...
   - `agent-memory-scrubber` owns curation of this capture, along with
     ...
     When `/sdlc:git-review-pr` spawns a reviewer standalone, outside
     that loop, there is no scrubber run after it, and this capture
     sits uncurated on the branch until something else curates it.
...
A recurring reviewer failure mode is asserting that file X
