...
are per-round working state, and they outlive every worktree the round
ran in — the run's terminal cleanup reaches none of the scratchpad.
The posted review remains this procedure's only thing the next round
reads.
...
predecessor's ids are read from the log to derive what is settled and
what is in flight, never to act on the child itself. Multiple sessions
run against one repo and share `.claude/worktrees/`, so a blanket kill
reaches into another session's work. Acting only on ids you spawned is
what keeps the scope provably correct.
...
**you** spawned it, so it is no longer mid-run and its worktree stops
carrying a live-PID lock the run's terminal cleanup would have to skip.
Append its stop either way — that
record is this round's evidence that the child was written off. A
predecessor instance's
...
spawned it, so it is no longer mid-run and its worktree stops carrying
a live-PID lock, and append its stop either way with
...
### Leave every worktree in place

You clean up nothing — not your children's worktrees, not your own,
not a predecessor instance's. A round ends at the posted review.

That is what makes a reviewer killed mid-fan-out leave exactly the
state a reviewer that returned leaves, so nothing downstream has to
tell the two apart. Removal is a whole-run concern rather than a
round's, and it belongs to
`/git-tools:git-cleanup-branches-and-worktrees`, invoked once at the
end of the run: in the orchestrated flow by `sdlc:orchestrate` → "The
run's one cleanup, at the end", and on the standalone path by the
human `/sdlc:git-review-pr` tells to run it. Restate none of that
skill's gates and none of its mechanics here.

That sweep gates on what each worktree holds, and your children hold
little: none of them ever claims the PR branch — each checks out
`origin/<branch>` detached (see their definitions), so there is no
branch claim to release and no local branch to delete — and none of
them commits. You take no branch claim either: you never check out the
PR branch attached.

`TaskStop` is unchanged by any of this: a child **you** spawned may be
stopped at its own deadline, a predecessor's never, per "You are
re-entrant".
...
Report any child whose `spawn` record got no `enter` too, by theorem
and stage. That child ran on nothing this round can name — a `spawn`
record carries no agent id and no path — so this line is the only
