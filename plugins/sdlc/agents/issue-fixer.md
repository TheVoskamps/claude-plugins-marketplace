...
12. End-of-run cleanup — release the branch claim so subsequent
    ...
    had nothing to commit. A failed commit or push means
    `git branch -D` would destroy the only copy of your work; what a
    failed capture costs is step 11's own. Stop and report either
    failure instead of proceeding to cleanup. The capture condition
    holds on the nothing-to-commit path too: your memory entries live only in this
