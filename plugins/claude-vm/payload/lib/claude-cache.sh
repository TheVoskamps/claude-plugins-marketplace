...
# The launcher then shares the cached binary into the guest, where the
# image's fstab mounts it `ro` (the RO is guest-side: vfkit's virtio-fs
# device has no read-only export, so the host exports every share
# read-write).
#
